	...
	<script>
		...
		function validateForm() {
			...
			if (cptName == null || cptName == "") {
				...
				var errName = document
						.createTextNode("Please enter a computer name. ou PAS");
				form.appendChild(image);
				form.appendChild(errName)
				//form.insertBefore(image, document.getElementById("nameSpan"));

				//form.insertBefore(errName, document.getElementById("nameSpan"));
				error = false;
			}

			if (cptName.match(nameExc)) {
				var coutInv = document.createElement("c:out");
				coutInv.setAttibute("value",
			...
			if ((!intDate.match(dateISO)) || (!isValidDate(intDate))) {
				...
				coutInt.setAttibute("value",
						"Please enter a valid introduced date.");
				form.insertBefore(image, document
						.getElementById("introducedSpan"));
				form.insertBefore(coutInt, document
						.getElementById("introducedSpan"));
				error = false;
				dateError = true;
			}
			if ((!discDate.match(dateISO)) || (!isValidDate(discDate))) {
				...
				coutDisc.setAttibute("value",
						"Please enter a valid discontinued date.");
				form.insertBefore(image, document
						.getElementById("discontinuedSpan"));
				form.insertBefore(coutDisc, document
						.getElementById("discontinuedSpan"));
				error = false;
				dateError = true;
			}
			if (!dateError) {
				if (discLTInt(intDate, discDate)) {
					...
					form.insertBefore(image, document
							.getElementById("discontinuedSpan"));
					form.insertBefore(coutLT, document
							.getElementById("discontinuedSpan"));
					error = false;
				}
			}
